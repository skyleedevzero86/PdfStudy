...
class KeywordSearchStrategy(
    ...
    override fun search(request: SearchRequest): SearchResponse {
        ...
        """.trimIndent()
        val countSql = """
            SELECT COUNT(*) FROM documents d WHERE $baseWhere
        """.trimIndent()
        val totalElements = jdbcTemplate.queryForObject(countSql, Long::class.java, request.query, request.docType, request.docType) ?: 0L
        ...
        }, request.query, request.docType, request.docType, request.size, request.page * request.size)
